...
data "archive_file" "get_account_transcripts" {
  ...
  output_path = "${path.module}/../pipelines/lambda/get_account_transcripts/get_account_transcripts.zip"
}

resource "aws_lambda_function" "get_account_transcripts" {
  function_name    = "vantagepoint-get-account-transcripts"
  role             = aws_iam_role.lambda.arn
  handler          = "lambda_function.lambda_handler"
  runtime          = "python3.11"
  timeout          = 30
  memory_size      = 256
  filename         = data.archive_file.get_account_transcripts.output_path
  source_code_hash = data.archive_file.get_account_transcripts.output_base64sha256
...
resource "aws_apigatewayv2_integration" "transcripts" {
  api_id                 = aws_apigatewayv2_api.main.id
  integration_type       = "AWS_PROXY"
  integration_uri        = aws_lambda_function.get_account_transcripts.invoke_arn
  payload_format_version = "2.0"
}
...
resource "aws_lambda_permission" "transcripts_apigw" {
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.get_account_transcripts.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_apigatewayv2_api.main.execution_arn}/*/*"
}
